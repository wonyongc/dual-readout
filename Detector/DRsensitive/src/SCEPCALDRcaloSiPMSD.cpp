...
drc::SCEPCALDRcaloSiPMSD::SCEPCALDRcaloSiPMSD(const std::string aName,
...

{
    ...
    fSeg = dynamic_cast<dd4hep::DDSegmentation::SCEPCALGridDRcalo*>( aSeg.segmentation() );
    fWavlenStep = (fWavlenStart-fWavlenEnd)/(float)fWavBin;
    fTimeStep = (fTimeEnd-fTimeStart)/(float)fTimeBin;
}


drc::SCEPCALDRcaloSiPMSD::~SCEPCALDRcaloSiPMSD() {}



void drc::SCEPCALDRcaloSiPMSD::Initialize(G4HCofThisEvent* hce) {
...
G4bool drc::SCEPCALDRcaloSiPMSD::ProcessHits(G4Step* step, G4TouchableHistory*) {
...
*/
    ...
    auto cID = fSeg->cellID(zero, zero, volID);
...
float drc::SCEPCALDRcaloSiPMSD::findWavCenter(G4double en) {
    int i = 0;
    for ( ; i < fWavBin+1; i++) {
...
float drc::SCEPCALDRcaloSiPMSD::findTimeCenter(G4double stepTime) {
    int i = 0;
    for ( ; i < fTimeBin+1; i++) {
